...
.grade-key{background:white;border-radius:8px;border:1px solid #e8e0d0;padding:10px 18px;margin-bottom:16px;display:flex;gap:18px;flex-wrap:wrap;align-items:center;font-size:12px;color:#555}
.grade-key strong{color:#111}
.btn-generate{width:100%;padding:16px;background:#F5A800;color:#1a1a1a;border:none;border-radius:9px;font-size:16px;font-weight:800;cursor:pointer;letter-spacing:.5px;transition:background .2s}
...
function rate(key,idx,val){ratings[key][idx]=ratings[key][idx]===val?0:val;renderStars(key,idx,ratings[key][idx]);updateAvg(key);}
function hov(key,idx,val,on){const cur=ratings[key][idx];const c=CORNERS.find(x=>x.key===key);document.querySelectorAll(`#stars_${key}_${idx} .star`).forEach((s,i)=>{const a=i<(on?val:cur);s.style.borderColor=a?c.color:"#ddd";s.style.background=a?c.color:"white";s.style.color=a?"white":"#aaa";});}
function renderStars(key,idx,val){const c=CORNERS.find(x=>x.key===key);document.querySelectorAll(`#stars_${key}_${idx} .star`).forEach((s,i)=>{const a=i<val;s.style.borderColor=a?c.color:"#ddd";s.style.background=a?c.color:"white";s.style.color=a?"white":"#aaa";});}
function updateAvg(key){const vals=Object.values(ratings[key]).filter(v=>v>0);document.getElementById(`avg_${key}`).textContent=vals.length?(vals.reduce((a,b)=>a+b,0)/vals.length).toFixed(1):"–";}
...
async function generateReport(){
  ...
  CORNERS.forEach(c=>{
    ratingsText+=`\n${c.label.toUpperCase()}:\n`;
    c.items.forEach((item,i)=>{const v=ratings[c.key][i];ratingsText+=`  • ${item}: ${v?`${v} – ${GRADES[v]}`:"Not rated"}\n`;});
  });
  try{
...
@app.route(”/logo”)
def serve_logo():
logo_path = os.path.join(os.path.dirname(**file**), “logo.png”)
...
styles = {
    ...
    "subtitle":sty("s",  fontName="Helvetica", fontSize=9, textColor=colors.HexColor("#cccccc"), alignment=TA_CENTER, leading=13),
...
for key, title, _, col, bg in CORNERS_DEF:
    ...
    hdr_tbl = Table([[
        ...
        Paragraph(f"<b>{avg:.1f}</b>", sty("ca", fontName="Helvetica-Bold", fontSize=22, textColor=WHITE, alignment=TA_RIGHT, leading=26)),
    ...
    for item, score in items_scores:
        ...
        rows.append([
            ...
            Paragraph(f'<font color="{col_hex}">{"●"*score}</font><font color="#cccccc">{"●"*(5-score)}</font>',
...
```
    age_guidance = {
        "U7/U8":  "This is a very young player (6-8 years old). Use very simple, fun, encouraging language. Focus entirely on enjoyment and basic fundamentals. Keep sentences short and celebratory.",
        "U9/U10": "This player is 8-10 years old. Use simple, warm, encouraging language. Focus on fun, basic skills and effort. Avoid complex tactical language.",
        "U11/U12":"This player is 10-12 years old. Use clear, encouraging language with some basic tactical awareness. Balance praise with simple development points.",
        "U13/U14":"This player is 12-14 years old. Use more detailed tactical and technical language. They can handle honest, constructive feedback alongside praise.",
        "U15/U16":"This player is 14-16 years old. Use mature, detailed tactical language. Be specific about technical and tactical development areas alongside strengths.",
    }

    prompt = f"""Write a formal FA Four Corner Model player review for Falmouth Community Youth Football Club.
```

Player: {name} | Position: {position} | Coach: {coach} | Team: {team} | Age Group: {agegroup} | Review: {season}
...
Rules: Use {name}’s first name throughout. Praise scores 4-5 warmly. Encourage development on scores 1-2. Match language complexity to the age group guidance above.”””
