...
.edit-section-header{padding:12px 18px;font-weight:800;font-size:13px;color:white;display:flex;align-items:center;gap:8px}
...
<div id="formScreen">
    ...
    <div class="card-body">
      ...
      <div class="grid2" style="margin-top:12px">
        <div>
          ...
          <select id="agegroup">
            <option>U7/U8</option><option>U9/U10</option>
            <option selected>U11/U12</option><option>U13/U14</option><option>U15/U16</option>
          </select>
        </div>
        <div>
  ...
  <div class="grade-key">
    <strong>Grade:</strong>
    <span><strong>1</strong> Needs Support</span><span><strong>2</strong> Developing</span>
    <span><strong>3</strong> Good</span><span><strong>4</strong> Very Good</span><span><strong>5</strong> Excellent</span>
  </div>
  ...
  <div class="card">
    ...
    <div class="card-body">
      ...
      <div class="grid2">
        <div><label>Key Strengths</label><textarea id="note_strengths" rows="2" placeholder="e.g. Great attitude..."></textarea></div>
...
async function downloadPDF(){
  ...
  try{
    const res=await fetch("/generate_pdf",{method:"POST",headers:{"Content-Type":"application/json"},
      body:JSON.stringify({...currentPayload, ...editedText})});
    if(!res.ok){const err=await res.json();throw new Error(err.error||"Server error");}
...
def build_pdf(data, output_path):
    ...
    for key, title, col, bg in CORNERS_DEF:
        ...
        for item, score in items_scores:
            ...
            dots_on  = '<font color="%s">%s</font>' % (col_hex, "●" * score)
            dots_off = '<font color="#cccccc">%s</font>' % ("●" * (5-score))
            rows.append([
                Paragraph("- %s" % item, styles["ri"]),
                Paragraph(dots_on + dots_off, sty("d", fontName="Helvetica", fontSize=10, leading=13)),
                Paragraph("<b>%d</b> <font color='#888'>%s</font>" % (score, label),
...
def get_ai_text(body):
    ...
    age_guidance = {
        "U7/U8":  "Very young player (6-8). Use simple, fun, celebratory language. Focus on enjoyment and basic fundamentals.",
        "U9/U10": "Young player (8-10). Use warm encouraging language. Focus on fun, basic skills and effort.",
        "U11/U12":"Player is 10-12. Clear encouraging language with some tactical awareness. Balance praise with simple development points.",
        "U13/U14":"Player is 12-14. More detailed tactical language. Honest constructive feedback alongside praise.",
        "U15/U16":"Player is 14-16. Mature detailed tactical language. Specific technical and tactical development areas.",
    }
...
Use %s's first name. Praise scores 4-5. Encourage development on 1-2. Match language to age group.""" % (
        name, position, coach, team, agegroup, season,
        age_guidance.get(agegroup,""),
        ratings_text,
        ("Coach comment: " + note_comment + "\n") if note_comment else "",
        ("Key strengths: " + note_strength + "\n") if note_strength else "",
        ("Development focus: " + note_develop + "\n") if note_develop else "",
        name
    )
    ...
    print("AI response: " + raw[:200], flush=True)
...
def generate_text():
    try:
        ...
        return jsonify({"error": "AI response error - please try again"}), 500
...
def generate_pdf():
    try:
        ...
        def parse_scores(corner_key):
            ...
            scores = []
            for item in items:
                pattern = re.compile(re.escape("* " + item) + r":\s*(\d)", re.IGNORECASE)
